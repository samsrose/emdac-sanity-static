...
const getQueryForType = (type: string): string => {
  switch (type) {
    case 'author':
...
const log = (msg: string, error?: boolean): void =>
...
export default async function revalidate(
  ...
  const jsonBody = JSON.parse(body) as { _id?: unknown; _type?: string }
  ...
  const slug = await sanityClient.fetch<string | string[]>(
  ...
  try {
    ...
    log(updatedRoutes)
    res.status(200).json({ message: updatedRoutes })
    return
  } catch (err) {
