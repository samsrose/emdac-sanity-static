...
import type { Member, Associate } from '../lib/types'
...
export default function Directory({ allMembers, allAssociates, preview }: DirectoryPageProps) {
        ...
        <div className="bg-gradient-to-b to-gray-800/50 from-gray-900">
          <div className="mx-auto px-6 lg:px-8">
            ...
            <h1 className='p-4 text-gray-200 leading-4 text-3xl text-center'>Members</h1>
              <div className="mx-auto w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:gap-x-4 gap-4 lg:gap-x-4 gap-y-4 md:gap-y-4 mb-12">
                <IsMemberDirectory data={moreMembers} />
              </div>
              <h1 className='p-4 text-gray-200 leading-4 text-4xl text-center'>Associates</h1>
              <div className="gap-4 mb-[1rem] mx-auto w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:gap-x-4 lg:gap-x-4 gap-y-4 gap-4 md:gap-y-4 mb-12">
...
export const getStaticProps: GetStaticProps<DirectoryPageProps> = async ({ preview = false }) => {
  const allMembers: Member[] = overlayDrafts(await getClient(preview).fetch(membersQuery))
  const allAssociates: Associate[] = overlayDrafts(await getClient(preview).fetch(associatesQuery))
  return {
