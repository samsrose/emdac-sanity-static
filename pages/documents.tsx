...
import type { DocumentRecord, EvidenceRecord, LegislativeRecord, Minute, Position, Protocol } from '../lib/types'
...
interface DocumentsPageProps {
  ...
  allLegislative: LegislativeRecord[]
  allMinutes: Minute[]
...
function TableItem({ data }: { data: TableRow[] }) {
  return (
      <>
...
export default function Documents({ allDocuments, allEvidence, allLegislative, allMinutes, allPositions, allProtocols, preview }: DocumentsPageProps) {
...
export const getStaticProps: GetStaticProps<DocumentsPageProps> = async ({ preview = false }) => {
  const allDocuments: DocumentRecord[] = overlayDrafts(await getClient(preview).fetch(documentsQuery))
  const allEvidence: EvidenceRecord[] = overlayDrafts(await getClient(preview).fetch(evidenceQuery))
  const allLegislative: LegislativeRecord[] = overlayDrafts(await getClient(preview).fetch(legislativeQuery))
  const allMinutes: Minute[] = overlayDrafts(await getClient(preview).fetch(minutesQuery))
  const allPositions: Position[] = overlayDrafts(await getClient(preview).fetch(positionsQuery))
  const allProtocols: Protocol[] = overlayDrafts(await getClient(preview).fetch(protocolsQuery))
