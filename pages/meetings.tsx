...
export default function MeetingPage({ allMeetings, preview }: MeetingPageProps) {
    ...
    <>
      ...
      <div className="h-screen bg-gray-900 w-full">
        ...
        <div className='bg-gradient-to-br from-gray-900 to-gray-800 flex align-center justify-center h-full max-w-7xl mx-auto'>
            <div className="divide-y divide-gray-900 overflow-hidden rounded-lg bg-gray-900 shadow grid grid-cols-2 md:grid-cols-1 lg:grid-cols-2 sm:gap-4 sm:divide-y-0">
...
export const getStaticProps: GetStaticProps<MeetingPageProps> = async ({ preview = false }) => {
  const allMeetings: Meeting[] = overlayDrafts(await getClient(preview).fetch(meetingsQuery))
